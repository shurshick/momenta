...
async def log_audit(db: AsyncSession, actor_user_id: Optional[uuid.UUID], action: str, entity_type: str,
                    entity_id: Optional[uuid.UUID] = None, ip_address: Optional[str] = None,
                    user_agent: Optional[str] = None, payload: Optional[dict] = None):
    log = AuditLog(
        id=uuid.uuid4(),
...
@router.post("/login")
async def admin_login(request: Request, username: str = Form(...), password: str = Form(...), db: AsyncSession = Depends(get_db)):
...
@router.get("/", response_class=HTMLResponse)
async def admin_dashboard(request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    total_users = (await db.execute(select(func.count(User.id)))).scalar() or 0
    today = date.today()
    posts_today = (await db.execute(select(func.count(Post.id)).where(Post.challenge_date == today, Post.status == "active"))).scalar() or 0
...
async def admin_users(request: Request, search: Optional[str] = Query(None), db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
...
async def admin_toggle_user_status(user_id: str, request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    user = await get_user_by_id(db, uuid.UUID(user_id))
    if not user:
        raise HTTPException(status_code=404)
    user.status = "disabled" if user.status == "active" else "active"
    await log_audit(db, admin.id, "toggle_user_status", "user", user.id, request.client.host if request.client else None, request.headers.get("user-agent"), {"new_status": user.status})
    await db.commit()
    return RedirectResponse(url="/admin/users", status_code=303)


@router.post("/users/{user_id}/set-role")
async def admin_set_user_role(user_id: str, role: str = Form(...), request: Request = None, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    if not user:
        raise HTTPException(status_code=404)
    user.role = role
    await log_audit(db, admin.id, "set_user_role", "user", user.id, request.client.host if request.client else None, request.headers.get("user-agent"), {"new_role": role})
    await db.commit()
    return RedirectResponse(url="/admin/users", status_code=303)
...
async def admin_create_challenge(request: Request, challenge_date: str = Form(...), title_ru: str = Form(...),
                                 description_ru: Optional[str] = Form(None), status: str = Form("draft"),
                                 db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    try:
        d = date.fromisoformat(challenge_date)
        challenge = await create_challenge(db, d, title_ru, description_ru, status=status, created_by=admin.id)
        await log_audit(db, admin.id, "create_challenge", "challenge", challenge.id, request.client.host if request.client else None, request.headers.get("user-agent"))
...
async def admin_create_fallback_challenge(request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    today = date.today()
    existing = await get_challenge_by_date(db, today)
    if existing:
        return RedirectResponse(url="/admin/challenges", status_code=303)
    challenge = await create_challenge(db, today, "Момент дня", "Запечатли свой момент сегодня", status="active", created_by=admin.id)
    await log_audit(db, admin.id, "create_fallback_challenge", "challenge", challenge.id)
    return RedirectResponse(url="/admin/challenges", status_code=303)


@router.get("/posts", response_class=HTMLResponse)
async def admin_posts(request: Request, status_filter: Optional[str] = Query(None), date_filter: Optional[str] = Query(None),
                      db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
...
async def admin_hide_post(post_id: str, request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    result = await db.execute(select(Post).where(Post.id == uuid.UUID(post_id)))
    post = result.scalar_one_or_none()
    if not post:
        raise HTTPException(status_code=404)
    post.status = "hidden"
    await log_audit(db, admin.id, "hide_post", "post", post.id, request.client.host if request.client else None, request.headers.get("user-agent"))
    await db.commit()
    return RedirectResponse(url="/admin/posts", status_code=303)


@router.post("/posts/{post_id}/restore")
async def admin_restore_post(post_id: str, request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    result = await db.execute(select(Post).where(Post.id == uuid.UUID(post_id)))
    post = result.scalar_one_or_none()
    if not post:
        raise HTTPException(status_code=404)
    post.status = "active"
    await log_audit(db, admin.id, "restore_post", "post", post.id, request.client.host if request.client else None, request.headers.get("user-agent"))
    await db.commit()
    return RedirectResponse(url="/admin/posts", status_code=303)


@router.get("/reports", response_class=HTMLResponse)
async def admin_reports(request: Request, status_filter: Optional[str] = Query(None), db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
...
async def admin_resolve_report(report_id: str, action: str = Form(...), request: Request = None,
                                db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    await log_audit(db, admin.id, f"resolve_report_{action}", "report", report.id, request.client.host if request.client else None, request.headers.get("user-agent"))
    await db.commit()
...
async def admin_flush_feed(request: Request, db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    from app.services.redis_service import flush_feed_cache
    await flush_feed_cache(date.today())
    await log_audit(db, admin.id, "flush_feed_cache", "system", ip_address=request.client.host if request.client else None, user_agent=request.headers.get("user-agent"))
