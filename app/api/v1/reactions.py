...
from app.services.post_service import like_post as like_post_service, unlike_post as unlike_post_service
...
async def like_post(post_id: str, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
    return await like_post_service(db, uuid.UUID(post_id), uuid.UUID(user_id))


@router.delete("/{post_id}/like")
async def unlike_post(post_id: str, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
    return await unlike_post_service(db, uuid.UUID(post_id), uuid.UUID(user_id))
