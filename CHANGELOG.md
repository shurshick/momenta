...
- **Android**: лучший момент дня загружается независимо от карточки задания и показывается даже если задание временно не пришло.

## [v0.2.24] - 2026-06-27

### Changed
- **Android**: профиль обновляется тихо при возврате на экран, без полного лоадера и заметного моргания страницы.
- **Android**: главный экран повторно обновляет лучший момент дня при возврате на вкладку.
- **Android**: кнопка скачивания APK теперь скачивает файл в кэш приложения и открывает системный установщик, а не просто браузер.
- **Release**: APK в GitHub Actions подписывается стабильным update-keystore из секрета, чтобы следующие версии ставились поверх предыдущих.
- **Backend**: лучший момент дня использует fallback по активным постам за последние 48 часов, если за текущую дату ничего не найдено.
