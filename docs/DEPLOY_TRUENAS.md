...
Начиная с `v0.2.70` контейнер API выполняет `alembic upgrade head` до запуска FastAPI. Это работает и для чистой установки, и для обновления существующей базы.

После запуска проверьте ревизию:
...
```bash
docker logs --tail 200 momenta-api
```

Ручной запуск остаётся аварийным вариантом:
...
1. Замените тег образа, например на `ghcr.io/shurshick/momenta:v0.2.71`.
